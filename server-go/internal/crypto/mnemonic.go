// Mnemonic 12 words: генерация при регистрации. Сид возвращается клиенту один раз
// и никогда не записывается в БД. Приватный ключ выводится только в RAM из сида.
package crypto
...
var ErrMnemonicEntropySize = errors.New("crypto: entropy must be 16 bytes for 12-word mnemonic")
...
// GenerateMnemonic12 создаёт 12 слов (128 бит). Возвращать пользователю при регистрации;
// в store/БД не сохранять. Из сида ключи выводятся только в памяти клиента/сервера.
...
// EntropyToMnemonic12 превращает 16 байт энтропии в 12 слов (BIP39-совместимый формат).
func EntropyToMnemonic12(entropy []byte) ([]string, error) {
	if len(entropy) != EntropySize128 {
		return nil, ErrMnemonicEntropySize
	}
	checksum := sha256.Sum256(entropy)
	full := make([]byte, EntropySize128+1)
	copy(full, entropy)
	full[EntropySize128] = checksum[0]
	words := make([]string, MnemonicWords12)
	for i := 0; i < MnemonicWords12; i++ {
		...
		if int(idx) >= len(bip39WordList) {
			idx = uint16(int(idx) % len(bip39WordList))
		}
...
// SeedFromMnemonic12 возвращает 64-байтный seed из 12 слов (passphrase опционален).
// Использовать только для вывода ключей в RAM; результат не сохранять.
func SeedFromMnemonic12(words []string, passphrase string) []byte {
	sentence := strings.Join(words, " ")
	return pbkdf2.Key([]byte(sentence), []byte("mnemonic"+passphrase), 2048, 64, sha256.New)
}

// bip39WordList — первые 2048 слов BIP39 (сокращённый список для размера; в проде можно полный).
