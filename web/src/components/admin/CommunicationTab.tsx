...
export default function CommunicationTab() {
  ...
  const createInvite = async () => {
    try {
      const d = await api('/api/admin/invite-links', 'POST', { maxUses: 0 });
      showToast('Создано: ' + (d?.url || d?.code), 'success');
  ...
  return (
    <div>
      ...
      {section === 'invites' && (
        <div>
          <button onClick={createInvite} style={{ marginBottom: 16, padding: '10px 20px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer' }}>Создать ссылку</button>
          {invites.length === 0 ? <div className="empty" style={{ padding: 48 }}>Нет ссылок</div> : (
            <div style={{ display: 'grid', gap: 8 }}>
              {invites.map((i) => (
                <div key={i.id} style={{ padding: 12, background: 'var(--panel-2)', border: '1px solid var(--border)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
